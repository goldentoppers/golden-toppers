...
export const IngredientPantry: React.FC<PantryProps> = ({
    ...
    return (
        <div id="ingredient-pantry-section" className="flex min-w-0 w-full flex-col">
            <div className="mt-2 grid gap-8 lg:grid-cols-[minmax(0,0.6fr)_minmax(0,1.4fr)] lg:items-start">
                <div className="flex flex-col gap-4">
                    <IngredientCategoryHeader activeChapter={chapterConfig} />
                    {details}
                </div>
                <div
                    className="flex flex-col gap-4 rounded-2xl border border-stone-900/8 bg-white/60 p-3
            ...
            sm:grid-cols-5 md:grid-cols-5 lg:grid-cols-7"
                ...
                {action && (
                    <div className="flex w-full flex-col gap-2 xs:flex-row xs:justify-between lg:col-start-2 lg:row-start-2">
                        {action}
                    </div>
                )}
            </div>
